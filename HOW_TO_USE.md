...
5. **Split-quality self-check**. Before presenting to the participant, verify each item and fix issues yourself first:
   - **Granularity**: Any Collection with fewer than 3 rules? Consider merging it into an adjacent Collection or folding it in as a sub-scenario rather than keeping it standalone.
   - **Architecture-decision ownership**: Is there a decision that actually constrains multiple Collections (overall architecture choice, shared infrastructure, common data model) but has been placed inside a single Collection's "Converged Rules"? If so, lift it to `BATCH-OVERVIEW.md` — either under Cross-Collection Relationships or a new "Batch-Level Architecture Constraints" sub-section — and have each constrained Collection's Discussion Scope explicitly reference it.
   - **Cross-Collection decision owner**: Does the Global Pending Convergence List contain any P0 item that affects multiple Collections but has no owner? If so, it must be placed in some Collection's Architecture Decision section, and other affected Collections must note "depends on Cxx#decision-name".
   - **⚠ classification**: Do the Pending Convergence Points mix in items that can be deferred to Tech Design or v2? Tag each point: `[needs-decision]` (participant must decide), `[defer-TD]` (decide at Tech Design), `[defer-v2]` (defer to phase 2). Only `[needs-decision]` items block convergence.
   - **Relationship-diagram depth**: Does the Cross-Collection diagram use only "prerequisite / invocation" edges? Check whether "data-sharing" (shared order / store / user model) or "constraint" (one decision constrains another Collection's design space) edges are missing.
   - **Assumption consistency**: Do the Given/When/Then rules across Collections share implicit premises (e.g., "network available", "device bound", "user logged in")? If so, declare them explicitly as batch-level assumptions in BATCH-OVERVIEW so Collections don't each invent their own.
6. **🛑 Pause for participant review**. Present to the participant:
