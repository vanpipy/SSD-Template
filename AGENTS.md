...
| 层 | 起始 | 终态 | 触发 |
|----|------|------|------|
| PRD | Draft | Active | Step 1 完成所有完成门 |
| PRD | Active | Implemented | **人工/agent 回写**(代码仓完成) |
| Tech Design | Draft | Ready | Step 2 完成所有完成门 |
| Plan | Draft | Ready | Step 3 完成所有完成门 |
...
> wiki 只检查**规格侧**的完成门。**实现侧**(代码/Lint/测试)的完成门见各代码仓 AGENTS。
...
**每个 Tech Design** (7 文件):
- [ ] **7 个子文件** 全部按 `schema/tech_design/` 完成门通过
...
> **实现侧完成门**(在代码仓 AGENTS,不在这里): 代码实现、类型检查、Lint、测试通过
...
**必须保留**:
- 维度 3 全部(3 步工作流 + 命名 + 状态 + 完成门 + 禁止行为 + 快速定位)
- 维度 1 / 2 的小节结构
...
不启用时: 保持 3 步工作流,完成门由 wiki + 代码仓的静态检查保障。
